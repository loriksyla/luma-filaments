...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, onClearCart, total, cartItems }) => {
    ...
    return (
        ...
        >
            <div
                className={`bg-white dark:bg-slate-900 rounded-2xl w-full max-w-2xl shadow-2xl border border-slate-200 dark:border-slate-800 my-8 overflow-hidden transition-all duration-300 transform ${isOpen ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-4 opacity-0'}`}
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {user.addresses.map(addr => (
                                            <div
                                                ...
                                                className={`cursor-pointer p-4 rounded-xl border-2 transition-all flex items-start gap-4 ${selectedAddressId === addr.id
